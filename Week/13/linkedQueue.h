...
class linkedQueueType: public queueADT<Type>
{
  public:
    ...
    void moveNthFront(const int queueElement);
...
template <class Type>
void linkedQueueType<Type>::moveNthFront(const int moveElement)
{
  nodeType<Type> *trail, *current;
  trail = this->queueFront;
  current = this->queueFront;
  for (int i = 1; i < moveElement; i++) {
    trail = current;
    current = current->link;
  }
  trail->link = trail->link->link;
  current->link = this->queueFront;
  this->queueFront = current;
}
